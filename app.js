...
let dataPutaran = []; let dataPeserta = []; let dataTrans = []; let currentUser = null;
let currentWheelWinner = null; let currentWheelPutaranId = null;
...
function getEmptyState(text) { return `<div class="empty-state"><h3>Data Kosong</h3><p>${text}</p></div>`; }
...
function playTick() {
    ...
    gain.gain.setValueAtTime(0.2, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.05);
...
function checkAuth() {
    ...
    if(currentUser.role === 'admin') {
        document.getElementById("menu-admin").style.display = "block"; window.navTo('s-dash', 'Dashboard', 'm-dash'); loadAdmin();
    } else {
...
document.getElementById("form-login").addEventListener("submit", async (e) => {
    ...
    if(telp === 'admin' && pin === 'admin123') { sessionStorage.setItem("loggedUser", JSON.stringify({ role: 'admin', nama: 'Administrator Pusat' })); window.location.reload(); return; }
...
onSnapshot(colPemenang, snap => {
    if(!snap.empty) {
        const win = snap.docs.map(d=>d.data()).sort((a,b) => (b.timestamp?.toMillis()||0) - (a.timestamp?.toMillis()||0))[0];
...
function loadAdmin() {
    ...
    window.siapkanEditPutaran = () => {
        ...
        document.getElementById("eput-nama").value = p.nama.split(" - ")[1] || p.nama; // ambil nama bersihnya
        document.getElementById("modal-putaran").style.display = "flex";
    };
    ...
    window.hapusSemuaPutaran = async () => {
        if(!window.verifyPin()) return alert("Aksi Dibatalkan!");
        for(let p of dataPutaran) await deleteDoc(doc(db, "putaran", p.id)); alert("Seluruh putaran direset.");
    };
    ...
    function renderPutaran() {
        ...
        document.getElementById("list-putaran").innerHTML = dataPutaran.map(p => `
            <div class="list-item" onclick="bukaDetailPutaran('${p.id}')">
                <div class="list-info"><h4>${p.nama}</h4><p>Ketuk untuk Opsi</p></div>
            </div>`).join('');
    }

    function populatePutaranCS() {
        let optCS = [{value: 'ALL', label: '>> Lihat Semua Putaran <<'}]; dataPutaran.forEach(p => optCS.push({value: p.id, label: p.nama}));
        window.initCustomSelect('cs-filter-putaran', optCS, renderSetoranList);
    ...
    function populatePesertaCS() {
        let optCS = [{value: '', label: 'Semua Peserta'}]; dataPeserta.forEach(p => optCS.push({value: p.id, label: p.nama}));
    ...
    window.bukaDetailPeserta = (id) => {
        ...
        document.getElementById("list-detail-bayar").innerHTML = html || "<p style='text-align:center;'>Belum ada setoran.</p>";
    ...
    window.aksiHapusPeserta = async () => {
        ...
        if(confirm(`Yakin hapus ${p.nama} dari sistem SELAMANYA? (Data transaksi tetap ada namun tanpa pemilik)`)){
            await deleteDoc(doc(db, "peserta", id));
            document.getElementById("modal-detail-peserta").style.display = "none"; alert("Peserta dihapus permanen.");
        }
    };

    function renderPeserta() {
        if(dataPeserta.length === 0) return document.getElementById("list-peserta").innerHTML = getEmptyState("Belum ada peserta.");
        document.getElementById("list-peserta").innerHTML = dataPeserta.map(p => `
            <div class="list-item" onclick="bukaDetailPeserta('${p.id}')">
                <div class="list-info"><h4>${p.nama}</h4><p>${p.telp} | PIN: ${p.pin}</p>
    ...
    function renderSetoranList() {
        ...
        dataPeserta.forEach(p => {
            ...
            listHtml += `
                <div class="list-item" ${onClickAttr} style="${pid==='ALL'?'cursor:default':''}">
                    <div class="list-info"><h4>${p.nama}</h4><p>${pid === 'ALL' ? 'Mode Lihat Semua (Tidak bisa input)' : 'Sentuh untuk input setoran'}</p></div>
                    <div class="amount">${status}</div>
                </div>`;
        });
        document.getElementById("list-setoran-peserta").innerHTML = listHtml || getEmptyState("Data tidak ditemukan.");
    ...
    window.shareLaporanWA = () => {
        const pid = document.getElementById("select-putaran-setoran").value; const pName = pid === 'ALL' ? 'Semua Putaran' : dataPutaran.find(x=>x.id===pid)?.nama;
        let t = `*TAGIHAN M-ARISAN*\nPutaran: ${pName}\n\n`;
    ...
    // --- 4. FULL CRUD TRANSAKSI / LOG KAS ---
    window.bukaModalTambahTransaksi = () => { document.getElementById("modal-tarik-kas").style.display = "flex"; };
    document.getElementById("form-tarik-kas").addEventListener("submit", async(e) => {
        e.preventDefault();
        await addDoc(colTrans, { tipe: 'OUT', nominal: Number(document.getElementById("tk-nominal").value), keterangan: document.getElementById("tk-ket").value, timestamp: serverTimestamp() });
        document.getElementById("modal-tarik-kas").style.display = "none"; document.getElementById("form-tarik-kas").reset();
    });
    ...
    function renderLogKas() {
        ...
        let tIn = 0; let tOut = 0; let html = "";
        dataTrans.forEach(t => {
            if(fPut !== 'ALL' && t.id_putaran !== fPut && t.tipe === 'IN') return; // Output murni ga punya putaran
            if(fPes !== 'ALL' && t.id_peserta !== fPes && t.tipe === 'IN') return;

            if(t.tipe === 'IN') { tIn += t.nominal; html += `<div class="list-item" onclick="bukaEditTransaksi('${t.id}')"><div class="list-info"><h4>${t.nama_peserta}</h4><p>${formatDate(t.timestamp)}</p></div><div class="amount" style="color:var(--success);">+${rupiah(t.nominal)}</div></div>`; }
            else { tOut += t.nominal; html += `<div class="list-item" onclick="bukaEditTransaksi('${t.id}')"><div class="list-info"><h4>${t.keterangan}</h4><p>${formatDate(t.timestamp)}</p></div><div class="amount" style="color:var(--danger);">-${rupiah(t.nominal)}</div></div>`; }
        });
        document.getElementById("list-log-kas").innerHTML = html || getEmptyState("Kosong.");
        document.getElementById("kas-in").innerText = rupiah(tIn); document.getElementById("kas-out").innerText = rupiah(tOut); document.getElementById("kas-saldo").innerText = rupiah(tIn - tOut);
    }

    window.bukaEditTransaksi = (id) => {
        const t = dataTrans.find(x => x.id === id);
        document.getElementById("et-id").value = t.id;
        document.getElementById("et-ket").value = t.tipe === 'IN' ? `Setoran: ${t.nama_peserta}` : `Keluar: ${t.keterangan}`;
    ...
    document.getElementById("form-edit-transaksi").addEventListener("submit", async(e) => {
        ...
        document.getElementById("modal-detail-transaksi").style.display = "none"; alert("Transaksi Berhasil Diubah!");
    });
    ...
    window.aksiHapusTransaksi = async () => {
        ...
        document.getElementById("modal-detail-transaksi").style.display = "none"; alert("Transaksi Dihapus!");
    };
    ...
    window.cetakLaporan = () => {
        ...
        printWin.document.write('<h2>Buku Kas Transparan</h2><p>Dicetak: ' + new Date().toLocaleString('id-ID') + '</p>');
        printWin.document.write(document.getElementById('list-log-kas').innerHTML.replace(/<div class="list-item" onclick=".*?">/g, '<tr>').replace(/<div class="list-info"><h4>(.*?)<\/h4><p>(.*?)<\/p><\/div><div class="amount.*?">(.*?)<\/div><\/div>/g, '<td>$1</td><td>$2</td><td>$3</td></tr>').replace(/<div class="empty-state">.*?<\/div>/g, '<p>Data kosong</p>'));
        printWin.document.write('</body></html>');
        printWin.document.close(); printWin.print();
    };

    // --- 5. RODA UNDIAN HD (Real Physics) ---
    ...
    document.getElementById("btn-spin").addEventListener("click", () => {
        ...
        currentWheelPutaranId = document.getElementById("select-putaran-undi").value;
        ...
        const duration = 7000; let startTime = null; lastPassedSliceIndex = -1;
        ...
        function animateWheel(time) {
            ...
            if (t < 1) { animationFrameId = requestAnimationFrame(animateWheel); }
                ...
                playWin(); document.getElementById("btn-spin").disabled = false;
...
function loadUser() {
    ...
    onSnapshot(colTrans, snap => {
        ...
        myTrans.forEach(t => { total += t.nominal; html += `<div class="list-item"><div class="list-info"><h4>Setoran Berhasil</h4><p>${formatDate(t.timestamp)}</p></div><div class="amount" style="color:var(--success); font-weight:bold;">+${rupiah(t.nominal)}</div></div>`; });
    ...
    document.getElementById("form-profil").addEventListener("submit", async(e) => {
        ...
        if(file) {
            ...
            reader.onloadend = async () => { newData.fotoBase64 = reader.result; await updateDoc(doc(db, "peserta", currentUser.id), newData); alert("Profil tersimpan!"); document.getElementById("modal-profil").style.display = "none"; };
